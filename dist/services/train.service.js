...
export async function getLiveTrainStatus(trainNo, startDateParam, targetStationCode) {
    const trainsResponse = await getLiveTrains();
    const allMatches = trainsResponse.data.filter((train) => train.train_no === trainNo);
    ...
    let targetStationInfo = undefined;
    if (targetStationCode && details.train_public_time_table) {
        const normTarget = targetStationCode.trim().toUpperCase();
        const targetStop = details.train_public_time_table.find((s) => s.station_code === normTarget);
        if (targetStop) {
            let currentStop = details.train_public_time_table.find((s) => s.station_code === stationCode);
            ...
            if (!currentStop) {
                const departedStops = details.train_public_time_table.filter((s) => s.has_departed === "1" || s.has_departed === "Yes");
...
function parseISTDateTime(dateTimeStr) {
...
export async function getTrainsAtStation(stationCode, hours = 2) {
        ...
        if (fs.existsSync(stationTrainsPath)) {
            ...
            function isTimeInWindow(timeStr) {
                ...
                if (endMinutes > 1440) {
                    ...
                    if (stopMinutesNextDay >= startMinutes && stopMinutesNextDay <= endMinutes)
                        return true;
                }
                if (startMinutes < 0) {
                    const stopMinutesPrevDay = stopMinutes - 1440;
                    if (stopMinutesPrevDay >= startMinutes && stopMinutesPrevDay <= endMinutes)
                        return true;
                }
                return false;
            }
            const candidateStops = scheduledStops.filter((stop) => isTimeInWindow(stop.a) || isTimeInWindow(stop.d));
            scheduledTrainNos = candidateStops.map((stop) => stop.t);
        }
    }
    catch (err) {
    ...
    for (let i = 0; i < candidates.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (cand) => {
            try {
                ...
                if (!details || !details.train_public_time_table)
                    return;
                const liveStop = details.train_public_time_table.find((s) => s.station_code === normStation);
                if (!liveStop)
                    return;
