...
function formatTrainsAtStationSummary(
  stationCode: string,
  hours: number,
  trains: any[]
): string {
  ...
  for (const t of trains) {
    ...
    const isArrDelayed =
      t.delayArrival !== "On Time" &&
      t.delayArrival !== "00:00" &&
      t.delayArrival !== "";
    const isDepDelayed =
      t.delayDeparture !== "On Time" &&
    ...
    lines.push(
      `| ${trainInfo} | ${schedule} | ${liveStatus} | ${delay} | ${pf} | ${current} |`
    );
  }
...
export const getTrainsAtStationTool: McpTool<{
